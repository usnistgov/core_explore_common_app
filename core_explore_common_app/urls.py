...
urlpatterns = [
    ...
    re_path(r'^get_local_data_source', user_ajax.get_local_data_source,
            name='core_explore_common_get_local_data_source'),

    re_path(r'^update-local-data-source', user_ajax.update_local_data_source,
            name='core_explore_common_update_local_data_source'),

    re_path(r'^data-sources-html', user_ajax.get_data_sources_html,
            name='core_explore_common_data_sources_html'),

    re_path(r'^data-source-results/(?P<query_id>\w+)/(?P<data_source_index>\w+)/(?P<page>\w+)',
            user_ajax.get_data_source_results,
            name='core_explore_common_data_source_results'),

    re_path(r'^data-source-results/(?P<query_id>\w+)/(?P<data_source_index>\w+)',
            user_ajax.get_data_source_results,
            name='core_explore_common_data_source_results'),
]
